...
// Palette + type mirror the live website (src/styles.css :root), converted from
// OKLCH to email-safe hex (email clients don't support OKLCH) with web-safe
// fallback stacks. Aubergine base · cherry accent · blush-white text.
const C = {
  bg: "#16051a",     // --paper (deep aubergine)
  card: "#1d0b21",   // surface
  well: "#0d0111",   // darker band (amount / code)
  border: "#3a253d", // --border
  ink: "#f7eff6",    // --ink (blush white)
  muted: "#af9eaf",  // --muted-foreground (secondary text)
  clay: "#e9687e",   // --clay (cherry accent)
};
// Website fonts. Supporting clients (Apple Mail, iOS) load them via @import;
// the rest fall back down the stack. MONO carries the site's eyebrow idiom.
const SERIF = "'Instrument Serif',Georgia,'Times New Roman',serif";
const SANS = "'Inter','Helvetica Neue',Helvetica,Arial,sans-serif";
const MONO = "'JetBrains Mono','Courier New',monospace";
const FONT_IMPORT = `<style>@import url('https://fonts.googleapis.com/css2?family=Instrument+Serif&family=Inter:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap');</style>`;
...
export function buildOrderConfirmationHtml(data: OrderConfirmationData, rate: Rate): string {
  ...
  // Item ledger — left-aligned, hairline-separated rows. Meta in mono (site idiom).
  const itemRows = data.items
    .map((i, idx) => {
      ...
      return `<tr>
        ...
        <td valign="top" style="padding:15px 14px;${topRule}">
          <div style="font-family:${SANS};font-size:15px;color:${C.ink};line-height:1.35;">${esc(i.name)}</div>
          <div style="margin-top:6px;font-family:${MONO};font-size:10px;letter-spacing:0.14em;text-transform:uppercase;color:${C.muted};">${esc(i.size)} &nbsp;·&nbsp; ${esc(i.colour)} &nbsp;·&nbsp; Qty ${i.quantity}</div>
        </td>
        <td valign="top" align="right" style="padding:15px 0;${topRule}font-family:${MONO};font-size:13px;color:${C.ink};white-space:nowrap;">${money(i.unitPrice * i.quantity)}</td>
      </tr>`;
    })
    .join("");

  // Totals ledger — mono labels + mono figures; the Total lands in serif on a clay rule
  const row = (label: string, value: string, strong = false): string => `
    <tr>
      <td style="font-family:${MONO};font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:${strong ? C.ink : C.muted};padding:${strong ? "15px 0 0" : "6px 0"};${strong ? `border-top:2px solid ${C.clay};` : ""}">${label}</td>
      <td align="right" style="font-family:${strong ? SERIF : MONO};font-size:${strong ? "26px" : "13px"};color:${C.ink};padding:${strong ? "9px 0 0" : "6px 0"};${strong ? `border-top:2px solid ${C.clay};` : ""}">${value}</td>
    </tr>`;
    ...
    ? `<tr><td style="padding:2px 40px 30px;">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-top:1px solid ${C.border};">
          <tr><td style="padding-top:22px;">
            <p style="margin:0 0 10px;font-family:${MONO};font-size:9px;letter-spacing:0.22em;text-transform:uppercase;color:${C.muted};">Shipping to</p>
  ...
  // theme: site tokens (aubergine/cherry · Instrument Serif + Inter + JetBrains Mono)
  // pre-emit critique: P5 H5 E4 S5 R5 V5 · email-adapted (tables/inline/hex)
  const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>${FONT_IMPORT}</head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" role="presentation" style="width:600px;max-width:92%;background:${C.card};border:1px solid ${C.border};">
        ...
        <tr><td style="padding:30px 40px 22px;border-bottom:1px solid ${C.border};">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
            <td valign="middle" style="font-family:${SERIF};font-size:26px;letter-spacing:0.14em;text-transform:uppercase;color:${C.ink};">Notteshe</td>
            <td valign="middle" align="right" style="font-family:${MONO};font-size:9px;letter-spacing:0.18em;text-transform:uppercase;color:${C.muted};line-height:1.6;">Order<br/><span style="color:${C.clay};font-size:12px;letter-spacing:0.08em;">#${ref}</span></td>
          </tr></table>
        </td></tr>

        <!-- Statement (left) -->
        <tr><td style="padding:36px 40px 4px;">
          <p style="margin:0 0 14px;font-family:${MONO};font-size:10px;letter-spacing:0.26em;text-transform:uppercase;color:${C.clay};">Order confirmed</p>
          <h1 style="margin:0 0 12px;font-family:${SERIF};font-weight:400;font-size:38px;line-height:1.02;color:${C.ink};">Thank you, ${esc(data.firstName)}.</h1>
        ...
        <tr><td style="padding:28px 40px 0;">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
            <tr>
              <td colspan="2" style="font-family:${MONO};font-size:9px;letter-spacing:0.22em;text-transform:uppercase;color:${C.muted};padding-bottom:10px;border-bottom:1px solid ${C.border};">The pieces</td>
              <td align="right" style="font-family:${MONO};font-size:9px;letter-spacing:0.22em;text-transform:uppercase;color:${C.muted};padding-bottom:10px;border-bottom:1px solid ${C.border};">Amount</td>
            </tr>
            ${itemRows}
          </table>
        </td></tr>
        ...
        <tr><td style="padding:22px 40px 28px;border-top:1px solid ${C.border};">
          <p style="margin:0 0 6px;font-family:${MONO};font-size:9px;letter-spacing:0.2em;text-transform:uppercase;color:${C.muted};">Designed &amp; made in Albania</p>
          <p style="margin:0;font-family:${MONO};font-size:9px;letter-spacing:0.06em;color:${C.muted};">© Notteshe &nbsp;·&nbsp; <a href="https://notteshe.com" style="color:${C.clay};text-decoration:none;">notteshe.com</a></p>
        </td></tr>
...
export function buildGiftCardDelivery(data: GiftCardDeliveryData): { subject: string; html: string; plainText: string } {
  ...
  const fromBanner = isGift
    ? `<tr><td style="padding:30px 40px 0;">
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-bottom:1px solid ${C.border};"><tr><td style="padding-bottom:26px;">
          <div style="font-family:${MONO};font-size:9px;letter-spacing:0.28em;text-transform:uppercase;color:${C.muted};margin-bottom:8px;">A gift from</div>
          <div style="font-family:${SERIF};font-size:34px;line-height:1;color:${C.ink};">${esc(data.senderName)}</div>
        </td></tr></table>
      </td></tr>`
    : "";
  ...
  const messageBlock = data.message
    ? `<tr><td style="padding:28px 40px 0;">
        <div style="padding-left:18px;border-left:2px solid ${C.clay};">
          <div style="font-family:${MONO};font-size:9px;letter-spacing:0.24em;text-transform:uppercase;color:${C.muted};margin-bottom:12px;">A note for you</div>
          <div style="font-family:${SERIF};font-size:23px;line-height:1.5;color:${C.ink};">&ldquo;${esc(data.message)}&rdquo;</div>
        </div>
      </td></tr>`
    : "";
  ...
  const step = (n: string, text: string): string =>
    `<tr>
      <td valign="top" width="36" style="padding:0 14px 16px 0;font-family:${SERIF};font-size:22px;color:${C.clay};white-space:nowrap;line-height:1.05;">${n}</td>
      <td valign="top" style="padding:0 0 16px;font-family:${SANS};font-size:12px;line-height:1.65;color:${C.muted};">${text}</td>
    </tr>`;

  // Hallmark · macrostructure: Statement/Poster · tone: warm/editorial
  // theme: site tokens (aubergine/cherry · Instrument Serif + Inter + JetBrains Mono)
  // pre-emit critique: P5 H5 E4 S5 R5 V5 · email-adapted (tables/inline/hex)
  const html = `<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>${esc(subject)}</title>${FONT_IMPORT}</head>
<body style="margin:0;padding:0;background:${C.bg};">
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:${C.bg};padding:40px 0;">
<tr><td align="center">
<table width="580" cellpadding="0" cellspacing="0" role="presentation" style="width:580px;max-width:92%;background:${C.card};border:1px solid ${C.border};">
  ...
  <!-- Masthead (left) -->
  <tr><td style="padding:30px 40px 22px;border-bottom:1px solid ${C.border};">
    <div style="font-family:${SERIF};font-size:26px;letter-spacing:0.14em;text-transform:uppercase;color:${C.ink};">Notteshe</div>
    <div style="margin-top:6px;font-family:${MONO};font-size:9px;letter-spacing:0.24em;text-transform:uppercase;color:${C.clay};">Grua e Fortë</div>
  </td></tr>

  <!-- Amount statement (dark band, left-aligned) -->
  <tr><td style="padding:46px 40px;background:${C.well};">
    <div style="font-family:${MONO};font-size:9px;letter-spacing:0.3em;text-transform:uppercase;color:${C.muted};margin-bottom:18px;">${isGift ? "You've received a gift card" : "Your gift card"}</div>
    <div style="font-family:${SERIF};font-size:66px;font-weight:400;line-height:0.9;color:${C.ink};">${formattedAmount}</div>
    <div style="margin-top:20px;height:2px;width:44px;background:${C.clay};font-size:0;line-height:0;">&nbsp;</div>
    <div style="margin-top:14px;font-family:${MONO};font-size:9px;letter-spacing:0.26em;text-transform:uppercase;color:${C.muted};">Notteshe Gift Card &nbsp;·&nbsp; No expiry</div>
  </td></tr>
  ...
  <tr><td style="padding:26px 40px 6px;">
    <div style="border:1px solid ${C.border};background:${C.well};padding:26px 28px;">
      <div style="font-family:${MONO};font-size:8px;letter-spacing:0.3em;text-transform:uppercase;color:${C.muted};margin-bottom:14px;">Your code</div>
      <div style="font-family:${MONO};font-size:24px;letter-spacing:5px;color:${C.ink};font-weight:500;">${esc(data.code)}</div>
    </div>
  </td></tr>

  <!-- How to redeem (numbered ledger) -->
  <tr><td style="padding:26px 40px 40px;">
    <div style="font-family:${MONO};font-size:9px;letter-spacing:0.28em;text-transform:uppercase;color:${C.muted};margin-bottom:20px;padding-bottom:12px;border-bottom:1px solid ${C.border};">How to redeem</div>
    <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
